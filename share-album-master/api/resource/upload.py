...
class UploadPicture(Resource):
    ...
    def get(self):
        ...
        if fp is not None:
            ...
            # 保存文件到服务器本地
            file = os.path.join(ori_dir,"static/img/images/%s.jpg" % uid)
            file_keep = os.path.join(ori_dir,"static/img/keep/%s.jpg" % uid)
            fp.save(file)
            ...
            with open(file, 'rb') as f:
                if len(f.read()) < 100:
                    ...
                    file2 = os.path.join(ori_dir,"static/img/images/%s.jpg" % uid2)
                    if len(out.mode) == 4:
                        r, g, b, a = out.split()
